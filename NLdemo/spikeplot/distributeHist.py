...
class Ui_MainWindow(QMainWindow):
    ...
    def plotDistribution(self):
        self.channelRates = self.trial_arr.mean(axis=0).sum(axis=1)
        if self.n_time > 0:
            s = 1000 / self.n_time
            self.channelRates *= s
        sc = MplCanvas(self.verticalLayoutWidget, width=4.8, height=3.6, dpi=100)
        sc.axes.hist(self.channelRates, bins=10, facecolor="blue", edgecolor="black", alpha=0.5)
        sc.axes.set_title("The distribution of firing rates of the neurons")
        sc.axes.set_xlabel("Firing rates(hz)")
        sc.axes.set_ylabel("Neuron counts")
